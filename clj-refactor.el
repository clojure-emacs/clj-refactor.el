...
(defun cljr--test-require-flag (req)
  (let ((t-req (s-trim req)))
...
(defun cljr--clean-request-element (req-elem)
  (replace-regexp-in-string "\\[?]?" "" req-elem))

(defun cljr--handle-req-statement (req req-elems)
  (save-excursion
    (goto-char (point-min))
    (let ((refer-index (-elem-index ":refer" req-elems))
	  referred)
      (cond ((s-match ":refer[[:space:]]+:all" req)
	     (format "%s :refer :all]" (car req-elems)))
	    ((or (cljr--test-require-flag (cljr--clean-request-element req))
		 (and (= 1 (safe-length req-elems))
		      (re-search-forward (cljr--req-element-regexp (cljr--clean-request-element (car req-elems)) "/") nil t)))
	     req)
	    ((and refer-index
		  (or (= refer-index 1) (= refer-index 3)))
	     (-map (lambda (elem)
		     (goto-char (point-min))
		     (let ((e (cljr--clean-request-element elem)))
		       (when (re-search-forward (cljr--req-element-regexp e "[^[:word:]^-]") nil t)
			 (push e referred))))
		   (nthcdr (1+ refer-index) req-elems))
	     (when referred
	       (format "%s [%s]]" (s-join " " (-take (1+ refer-index) req-elems))
		       (s-join " " (nreverse referred)))))
	    ((and (string= (nth 1 req-elems) ":as")
		  (re-search-forward (cljr--req-element-regexp (cljr--clean-request-element (nth 2 req-elems)) "/") nil t))
	     req)))))

(defun cljr--handle-prefix-list (req-elems)
  (let ((f-elem (cljr--clean-request-element (car req-elems)))
	 prefix-embedded-buffer
	 prefix-elems)
    (-map (lambda (elem)
	    (goto-char (point-min))
	    (cond ((and (not prefix-embedded-buffer)
			(not (s-starts-with? "[" elem)))
		   (when (re-search-forward (cljr--req-element-regexp (s-join "." (list f-elem (cljr--clean-request-element elem))) "/") nil t)
		     (push (cljr--clean-request-element elem) prefix-elems)))
		  ((and (s-ends-with? "]" elem)
			prefix-embedded-buffer)
		   (push (replace-regexp-in-string "]]]?" "]" elem) prefix-embedded-buffer)
		   (let* ((elems (nreverse prefix-embedded-buffer))
			  (result (cljr--handle-req-statement
				   (s-join " " elems)
				   elems)))
		     (push (when result (concat "\n" result))
			   prefix-elems))
		   (setq prefix-embedded-buffer nil))
		  ((or (s-starts-with? "[" elem)
		       prefix-embedded-buffer)
		   (push elem prefix-embedded-buffer))))
	  (nthcdr 1 req-elems))
    (format "[%s %s]" f-elem (s-join " " (nreverse (delq nil prefix-elems))))))

(defun cljr--test-require-used (statement)
  (save-excursion
    (let ((req-elems (split-string statement)))
      (if (or (= 1 (safe-length req-elems))
	      (string= ":refer" (nth 1 req-elems))
	      (string= ":as" (nth 1 req-elems)))
	  (cljr--handle-req-statement statement req-elems)
	(cljr--handle-prefix-list req-elems)))))

(defun cljr--remove-required (statements)
  (search-backward "(")
...
(defun cljr-remove-unused-requires ()
  ...
  (save-excursion
    (dolist (statement (->> (cljr--extract-ns-statements ":require")
			 (cljr--remove-required)
			 (-map 's-trim)
			 (-map 'cljr--test-require-used)
			 (delq nil)
                         (nreverse)))
      (cljr--insert-in-ns ":require")
      (insert statement)
