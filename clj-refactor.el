...
(defun cljr--looking-back (regexp &optional limit greedy)
  "Call `looking-back', but tolerate limit > point."
  (looking-back regexp (and limit
                            (min (point-min) limit))
                greedy))

(defun cljr--goto-toplevel ()
  (paredit-backward-up (cljr--depth-at-point))
  (when (cljr--looking-back "#")
    (backward-char)))
...
(defun cljr--more-namespaces-in-use-p (nth)
  ...
  (let ((use-end (save-excursion (forward-sexp) (point))))
    (prog1
      ...
      (if (and (cljr--looking-back "\\]") (looking-at "\\]"))
          (paredit-backward-up)
        (paredit-backward)))))
...
(defun cljr--find-symbol-at-point ()
  (save-excursion
    (when (cljr--looking-back "\\s_\\|\\sw" 1)
      (paredit-backward))
    (let ((beg (point)))
      (paredit-forward)
      (buffer-substring-no-properties beg (point)))))
...
(defun cljr-slash ()
  ...
  (when (and cljr-magic-requires
             (cljr--looking-back (cljr--magic-requires-re) (point-at-bol)))
  ...
  (dolist (symbol-meta (cljr--sort-occurrences occurrences))
    (let* ((file (gethash :file symbol-meta))
      ...
      (with-current-buffer (find-file-noselect file)
        ...
        (forward-line (1- line-beg))
        (forward-char (1- col-beg))
        (let* ((call-site? (cljr--looking-back "(\s*"))
               (sexp (if call-site?
