...
(defun cljr--prompt-for-namespace (candidates)
  "Selects namespace to require from a list of candidates.

This will auto-select the only candidate if `cljr-magic-requires'
is not set to `:prompt'. Shows user the language contexts (:clj,
cljs) the alias has been used in previously."
  (cond
      ...
      (dolist (elt candidates)
        (cl-destructuring-bind (alias-name alias-require lang-contexts) elt
          (puthash (if lang-contexts
                   ...
                   alias-index)))
      (let ((ns-require (completing-read "Add require: " alias-index)))
